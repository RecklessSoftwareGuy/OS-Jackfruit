...
static void timer_callback(struct timer_list *t)
{
    ...
    list_for_each_entry_safe(node, next, &monitored_tasks, list) {
        ...
        if (rss > (long)node->hard_limit_bytes) {
        ...
        if (rss > (long)node->soft_limit_bytes && !node->soft_limit_warned) {
...
static long monitor_ioctl(struct file *f, unsigned int cmd, unsigned long arg)
{
    ...
    if (cmd == MONITOR_REGISTER) {
        ...
        mutex_lock(&monitor_lock);
        list_add_tail(&node->list, &monitored_tasks);
        mutex_unlock(&monitor_lock);
     ...
     * ============================================================== */
    {
        ...
        mutex_lock(&monitor_lock);
        list_for_each_entry_safe(node, next, &monitored_tasks, list) {
            if (node->pid == req.pid && strncmp(node->container_id, req.container_id, CONTAINER_ID_LEN) == 0) {
                list_del(&node->list);
                kfree(node);
                found = 1;
                break;
